...
def stable_softmax_1d(logits):
    ...
    logits -= max_along_axis(logits, 0)
...
def stable_softmax_2d_rowwise(logits):
    ...
    logits = logits - max_along_axis(logits, 1)[:, None]
    exp = array_exp(logits)
    return exp / sum_axis1(exp)[:, None]
...
def get_batch(data, block_size, batch_size, rng):
    ...
    starts = sample_random_batch_offsets(len(data), block_size, batch_size, rng)
    X, Y = [], []
    for start in starts:
...
def vectorize_counts_add_at(vocab_size, data):
    ...
    cnts = allocate_count_matrix(vocab_size)
    np.add.at(cnts, (data[:-1], data[1:]), 1)
    return cnts.astype(np.int64)
...
def decode_generated_sequence(ids, itos):
    ...
    return ''.join(itos[token] for token in ids)
